    ...
    public var description: String {
        switch self {
            ...
            case .error(let tagError):
                switch tagError {
                    case NFCSDKError.TagNotValid:
                        return "Tag not valid."
                    case NFCSDKError.MoreThanOneTagFound:
                        return "More than 1 tags was found. Please present only 1 tag."
                    case NFCSDKError.ConnectionError:
                        return "Connection error. Please try again."
                    case NFCSDKError.InvalidMRZKey:
                        return "MRZ Key not valid for this document."
                    case NFCSDKError.ResponseError(let description, let sw1, let sw2):
